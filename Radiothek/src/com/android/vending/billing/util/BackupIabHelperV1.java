 ...
 * 
 */
public class BackupIabHelperV1 {
	 ...
	 */
	public BackupIabHelperV1(Context ctx, String base64PublicKey) {
...
/**
		...
		try {
			...
			if (querySkuDetails) {
				...
				if (r != BILLING_RESPONSE_RESULT_OK) {
					...
					inv = querySkuDetails(inv, moreItemSkus);
				}
			}
	...
	public void queryInventoryAsync(List<String> moreSkus,
		...
		do {
			...
			for (int i = 0; i < purchaseDataList.size(); ++i) {
				...
				if (Security.verifyPurchase(mSignatureBase64, purchaseData,
					...
					/**
					 * add the following for test, because
					 * "android.test.purchased" it is not a real item and has no
					 * signature returned by the server
					 */
	 ...
	 * @throws JSONException
	 */
	Inventory querySkuDetails(Inventory inv, List<String> moreSkus)
			throws RemoteException, JSONException {
		logDebug("Querying SKU details.");
		ArrayList<String> skuList = new ArrayList<String>();
		Bundle querySkus = new Bundle();

		Iterator<String> itrTemp = moreSkus.iterator();
		while (itrTemp.hasNext()) {
			String strTemp = itrTemp.next();
			skuList.addAll(Arrays.asList(strTemp));
		}
		...
		if (response == 0) {
			...
			for (String thisResponse : responseList) {
				...
				SkuDetails skuDetail = new SkuDetails(ITEM_TYPE_INAPP, object);
				inv.addSkuDetails(skuDetail);
			}
		}
		return inv;
	}
